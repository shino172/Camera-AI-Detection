...
def get_conn(retries=3, delay=2):
    ...
    # raise RuntimeError("❌ Không thể kết nối tới PostgreSQL sau nhiều lần thử.")
...
def hand2mouth_thread(pose_model):
    """
    Chạy phát hiện tay-đưa-miệng -> overlay trực tiếp lên frame (light alert).
    KHÔNG tạo log sự kiện trong DB.
    """
    while True:
        try:
...
# , lstm_model=None, scaler=None, lbl_map=None
# ---------------- Cigarette thread (main event logger) ----------------
def cigarette_thread(pose_model, cig_model=None):
    last_save = {}
    while True:
        try:
